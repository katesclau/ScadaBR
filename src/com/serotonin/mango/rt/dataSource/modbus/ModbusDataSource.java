...
abstract public class ModbusDataSource extends PollingDataSource implements
		...
		try {
			...
			for (DataPointRT dataPoint : dataPoints) {
				...
				if (result instanceof ExceptionResult) {
					...
					// A response, albeit an undesirable one, was received from
					...
					// Update the slave status. Only set to false if it is not
					// true already.
					if (!slaveStatuses
	...
	public void forcePointRead(DataPointRT dataPoint) {
		...
		System.out.println("Is initialized ? " + modbusMaster.isInitialized());
