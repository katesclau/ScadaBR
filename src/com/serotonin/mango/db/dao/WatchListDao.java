...
//        System.out.println("watchList >>> " + watchList);
        if (Common.getEnvironmentProfile().getString("db.type").equals("postgres")){  
            try {
                ...
                PreparedStatement preStmt = conn.prepareStatement("insert into watchLists (xid, userId, name) values (?,?,?)");
                preStmt.setString(1, watchList.getXid());
                preStmt.setInt(2, userId);
                preStmt.setString(3, watchList.getName());
                preStmt.executeUpdate();

                ResultSet resSEQ = conn.createStatement().executeQuery("SELECT currval('watchlists_id_seq')");
                resSEQ.next();
                    ...
                    if (Common.getEnvironmentProfile().getString("db.type").equals("postgres")){  
                        try {
                            Connection conn = DriverManager.getConnection(Common.getEnvironmentProfile().getString("db.url"),
                                                        Common.getEnvironmentProfile().getString("db.username"),
                                                        Common.getEnvironmentProfile().getString("db.password"));
                            PreparedStatement preStmt = conn.prepareStatement("insert into watchLists (xid, name, userId) values (?,?,?)");
                            preStmt.setString(1, watchList.getXid());
