                ...
                if (ml.getId() == Common.NEW_ID){
                    if (Common.getEnvironmentProfile().getString("db.type").equals("postgres")){                
                        try {
                            ...
                            PreparedStatement preStmt = conn.prepareStatement(MAILING_LIST_INSERT);
                            preStmt.setString(1,  ml.getXid());
                            preStmt.setString(2, ml.getName());
                            preStmt.executeUpdate();

                            ResultSet resSEQ = conn.createStatement().executeQuery("SELECT currval('mailinglists_id_seq')");
                            resSEQ.next();
