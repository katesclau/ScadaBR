...
public class DataSourceDao extends BaseDao {
    ...
    private void insertDataSource(final DataSourceVO<?> vo) {
        if (Common.getEnvironmentProfile().getString("db.type").equals("postgres")){                
            try {
                ...
                PreparedStatement preStmt = conn.prepareStatement("insert into dataSources (xid, name, dataSourceType, data) values (?,?,?,?)");
            ...
            vo.setId(doInsert("insert into dataSources (xid, name, dataSourceType, data) values (?,?,?,?)", new Object[] {
                    ...
                    Types.VARCHAR, Types.VARCHAR, Types.INTEGER, Common.getEnvironmentProfile().getString("db.type").equals("postgres") ? Types.BINARY: Types.BLOB }));
        }
    ...
    private void updateDataSource(final DataSourceVO<?> vo) {
        ...
        ejt.update("update dataSources set xid=?, name=?, data=? where id=?", new Object[] { vo.getXid(), vo.getName(),
                ...
                Common.getEnvironmentProfile().getString("db.type").equals("postgres") ? Types.BINARY: Types.BLOB, Types.INTEGER });
