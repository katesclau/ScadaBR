...
public class ViewDao extends BaseDao {
	...
	public List<View> getViews(int userId) {
		List<View> views = query(VIEW_SELECT + USER_ID_COND, new Object[] {
				userId, userId }, new ViewRowMapper());
		setViewUsers(views);
		return views;
	}
	...
	public List<IntValuePair> getViewNames(int userId) {
		return query("select id, name from mangoViews" + USER_ID_COND,
				new Object[] { userId, userId }, new IntValuePairRowMapper());
	}
	...
	void insertView(View view) {
                if (Common.getEnvironmentProfile().getString("db.type").equals("postgres")){                
                    try {
                        ...
                        PreparedStatement preStmt = conn.prepareStatement("insert into mangoViews (xid, name, background, userId, anonymousAccess, data) values (?,?,?,?,?,?)");
                    ...
                    view.setId(doInsert(
                                    ...
                                    new Object[] { view.getXid(), view.getName(),
                                                    ...
                                                    Types.INTEGER, Types.INTEGER, Common.getEnvironmentProfile().getString("db.type").equals("postgres") ? Types.BINARY: Types.BLOB }));
                }
	}
	...
	void updateView(View view) {
		ejt.update(
				...
				new int[] { Types.VARCHAR, Types.VARCHAR, Types.VARCHAR,
						Types.INTEGER, Common.getEnvironmentProfile().getString("db.type").equals("postgres") ? Types.BINARY: Types.BLOB, Types.INTEGER });
	}
